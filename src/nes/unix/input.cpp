...
void create_input_file()
{
	...
	fprintf(outfile, "P1B		x\n");
	fprintf(outfile, "P1START		_ENTER\n");
	fprintf(outfile, "P1SELECT	_BACKSPACE\n");
	...
	fclose(outfile);
#if 1 // AWH - Sync file to disk
	int fd = open(dirname, O_RDWR);
	fsync(fd);
	close(fd);
	sprintf(dirname, "%s", BES_FILE_ROOT_DIR);
