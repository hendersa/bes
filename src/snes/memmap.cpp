...
#include <numeric>
#include <assert.h>
#include <unistd.h> // AWH
#include <sys/types.h> // AWH
...
bool8 CMemory::SaveSRAM (const char *filename)
{
	...
	if (size)
	{
		...
		#endif
#if 1 // AWH
			fprintf(stderr, "AWH: syncing SRAM '%s'\n", sramName);
			fd = open(sramName, O_RDWR);
			fsync(fd);
			close(fd);
			sprintf(temp, "%s/%s/snes", BES_FILE_ROOT_DIR,
				BES_SRAM_DIR);
			fprintf(stderr, "AWH: syncing SRAM dir '%s'\n", temp);
			fd = open(temp, O_RDWR);
			fsync(fd);
			close(fd);
#endif // AWH
