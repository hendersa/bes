...
struct EmulatedSystem GBAemulator = {
  NULL,
  NULL,
  NULL,
  NULL,
...
int desktopWidth = 0;
int desktopHeight = 0;
Filter filter = kStretch1x; // AWH kStretch2x;
u8 *delta = NULL;

int filter_enlarge = 1; // AWH 2;
...
#define EXE_NAME "vbam"
  ...
  FILE *f = fopen(name, "r");
#else
  sprintf(buffer, "%s/%s", BES_FILE_ROOT_DIR, name);
...
#if 1 // AWH - Settings from the GBA setting dialog
  fprintf(stderr, "gbaForceSettings: %d\n", gbaForceSettings);
  switch (gbaForceSettings)
  {
    case GBA_FORCE_FS_SYNC_AUDIO:
      autoFrameSkip = 1;
      soundSetSampleRate(11025);
      break;
    case GBA_FORCE_NO_FS_AUDIO:
      autoFrameSkip = 0;
      soundSetSampleRate(11025);
      break;
    case GBA_FORCE_NO_FS_NO_AUDIO:
      autoFrameSkip = 0;
      soundSetSampleRate(11025);
      soundSetEnable(0);
      break;
...
#else
  sprintf(stateName, "%s/%s/gb/%s%d.sgm", BES_FILE_ROOT_DIR,
    BES_SAVE_DIR, filename, num+1);
#endif // AWH
  return stateName;
}
...
void sdlWriteState(int num)
{
  ...
  if(GBAemulator.emuWriteState)
    GBAemulator.emuWriteState(stateName);
#if 0 // AWH
  ...
  systemDrawScreen();
#else
  fd = open(stateName, O_RDWR);
  fsync(fd);
  close(fd);
  fprintf(stderr, "GB/GBA save state: '%s'\n", stateName);
  /* Sync the directory when the screenshot is saved */  
#endif // AWH
}
...
void sdlReadState(int num)
{
  ...
  stateName = sdlStateName(num);
  if(GBAemulator.emuReadState)
    GBAemulator.emuReadState(stateName);
...
#endif // AWH
  GBAemulator.emuWriteBattery(buffer);
#if 1 /* AWH - Sync the file system */
  int fd = open(buffer, O_RDWR);
  fsync(fd);
  close(fd);
  sprintf(buffer, "%s/%s/gb", BES_FILE_ROOT_DIR, BES_SRAM_DIR);
...
void sdlReadBattery()
{
  ...
  res = GBAemulator.emuReadBattery(buffer);
...
void sdlInitVideo() {
  ...
  if (srcWidth > 256)
  {
    destWidth = screen512->w;
    destHeight = screen512->h;
  }
  else
  {
    destWidth = screen256->w;
    destHeight = screen256->h;
  }
#endif // AWH
  ...
  if (filterPix)
    free(filterPix);
  filterPix = (u8 *)calloc(1, 2 * destWidth * destHeight);
  srcPitch = srcWidth*2 + 4;
#endif // AWH
...
void sdlPollEvents()
{
  ...
  if (paused) {
    if ( doPauseGui(filename, PLATFORM_GBA) )
      emulating = 0;
    paused = !paused;
...
int gba_main(/*int argc, char **argv*/const char *szFile)
{
#if 0 // AWH
  ...
  if(optind < argc) {
...
#if 1 // AWH
    sprintf(buf, "%s/%s/gb/%s", BES_FILE_ROOT_DIR, BES_ROM_DIR,
      filename);
    u32 len = strlen(buf);
    if (len > SYSMSG_BUFFER_SIZE)
    {
...
#endif // AWH
    bool failed = false;
    IMAGE_TYPE type = utilFindType(/* AWH szFile*/buf);
    if(type == IMAGE_UNKNOWN) {
      systemMessage(0, "Unknown file type %s", /* AWH szFile*/buf);
#if 0 // AWH
      exit(-1);
#else
      return(0);
    ...
    if(type == IMAGE_GB) {
      failed = !gbLoadRom(/* AWH szFile*/ buf);
      if(!failed) {
        gbGetHardwareType();
      ...
      int size = CPULoadRom(/* AWH szFile*/ buf);
      failed = (size == 0);
      if(!failed) {
    ...
    if(failed) {
      systemMessage(0, "Failed to load file %s", /* AWH szFile */buf);
#if 0 // AWH
      exit(-1);
#else
      return(0);
...
#if 0 // AWH
filter = kStretch2x;
destOffset = 160 /* (80 * 2) horizontal adjust*/ +
    76800 /* (60 * 2 * 640) vertical adjust */;
#endif // AWH
  if(cartridgeType == 0) {
    srcWidth = 240;
    srcHeight = 160;
...
#if 0 // AWH
filter = kStretch3x;
destOffset = 160 + 30720 /* 24 * 2 * 640 vertical adjust*/;
#endif // AWH
    }
...
#endif // AWH
      {
        GBAemulator.emuMain(GBAemulator.emuCount);
        if(rewindSaveNeeded && rewindMemory && GBAemulator.emuWriteMemState) {
		handleRewinds();
        }
...
void systemDrawScreen()
{
  ...
  if (srcWidth > 256) destPitch = 1024;
  else destPitch = 512;
  renderedFrames++;
#if 0 // AWH
  if (openGL)
  ...
  //screen = (u8*)surface->pixels + destOffset;
  //SDL_LockSurface(surface);
  if (ifbFunction)
...
#else
  sprintf(buffer, "%s/%s/gb/%s%02d.png", BES_FILE_ROOT_DIR, 
    BES_SAVE_DIR, filename, a);
  GBAemulator.emuWritePNG(buffer);
#endif
